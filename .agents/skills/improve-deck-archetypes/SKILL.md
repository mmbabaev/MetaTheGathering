...
4. Выбрать правильный слой изменения:
   ...
   - fuzzy и правила пунктов 3+ → только отдельный `macro_name`, не меняя `general_name`;
...
7. Сохранить полный фактический список названий выбранного турнира как отдельный параметризованный регрессионный тест `raw name → general_name`; не заменять его только синтетическими примерами. Дополнительно написать негативные fuzzy-тесты, проверить сохранение `Participant.archetype_id` и owner-only текст отдельно от клубного.
...
- Никогда не подменять пользовательский custom-архетип существующим архетипом через fuzzy. Fuzzy влияет только на `macro_name`; при сомнении оставлять его пустым.
