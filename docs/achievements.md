...
## 3. Ачивки (27 штук, 42 уровня) ✅ реализовано

**Основные** (12) описаны ниже, **колодные** (15) — в §3.8.
...
### 3.6b. Ещё четыре основные

- 💪 **Побеждён, но не сломлен** — отыграл все раунды турнира, проиграв почти всё
  (поражений ≥ раундов−1, минимум 3 раунда). Награда ровно за то поведение, которое нужно
  сбору меты: человек не ушёл после двух поражений, а доиграл.
- 🦄 **Рыборог** — сыграл и в Goldfish, и в Единороге.
- 🌚 **Последний герой** — записал колоду последним из всех (зеркало «Буду первый»).
- 🍬 **Сладкая месть** — обыграл самого неудобного соперника: того, с кем до этого дня было
  ≥2 встречи и отрицательный личный счёт. H2H считается по своим парингам, DataLens не нужен.

### 3.8. 🃏 Колоды — «сыграл на такой-то деке» (15 одноразовых + Коллекционер)

Отдельное семейство `family="deck"`: по одной ачивке на популярный архетип, названия
сленговые. Живут в `services/achievements/decks.py`.

🐍 УЖ (Terror) · 🤪 Crazy (Madness) · 👑 Царь (Tron) · ⚙️ Железяка (Affinity) · 🌿 Лесник (Elves) ·
🔥 Пиромант (Burn) · 🕵️ Шпион (Spy/Walls) · ⚔️ Крестоносец (White Aggro) · 🚪 Вратарь (Gates) ·
🧚 Крылья (Faeries) · 🦉 Фамильяр (Familiars) · 🗡 Клинок (Blade) · 🌋 Поджигатель (Wildfire) ·
💀 Жнец (Sacrifice) · 🪓 Лесоруб (Ponza/Ramp)

- Сопоставление идёт по **общему типу** (`deck_mapping.general_archetype`), а не по строке:
  «Blue Delver», «Dimir Terror» и «UB terror» — одна и та же ачивка. Для семейств (любой Tron,
  любой Affinity, любые Faeries) дополнительно ловим подстроку.
- Засчитываются все прошлые участия игрока, а не только сегодняшнее.
- 🃏 **Полиглот** I/II/III (5 / 10 / 15) — сколько разных колодных ачивок собрано.
- В текстовой полке колодные показываются **одной строкой** со счётчиком и парой ближайших
  целей: пятнадцать строк подряд ломают читаемость. В картинке полки их нет вовсе — там
  только основные восемь.

> Заметка: раньше в позиции «Мультикласса» стоял «Еретик» (3-1 на деке вне топ-меты). Он требовал отдельного
...
- отправка best-effort в `try/except`: сбой отчёта не должен ронять анонс турнира;
- `notified_at` проставляется после успешной отправки — повторный прогон джобы не пришлёт то же
  самое второй раз;
...
- **«Еретик»** (3-1 на деке вне топ-меты) — вернуть, когда появится динамический расчёт актуального
  топа (`GROUP BY general_name` за 90 дней вместо статичного `Archetype.meta_rank`).
