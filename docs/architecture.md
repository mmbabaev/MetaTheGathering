...
`SessionLocal` — обычный `sessionmaker`: **каждый вызов создаёт новую сессию**. Паттерн везде
одинаковый — открыть в начале задачи, закрыть в `finally`:
...
Почему не `scoped_session` (как было раньше): бот асинхронный и работает в одном потоке, а
scoped-сессия одна на поток — то есть импорт, джобы, нажатия кнопок и веб-запросы делили бы один
объект. `db.close()` в `finally` любой задачи закрывал сессию всем остальным, включая тех, кто в
этот момент висел на `await`. Записи после `await` при этом терялись молча: объект оказывался
detached, `commit()` проходил вхолостую **без ошибки**. Так пришёл дубль отбивки «сбор метагейма
завершён» (PR #169, разбор — в `docs/scheduler.md`). Scoped-семантика проекту и не нужна:
`SessionLocal.remove()` не вызывается нигде.

Два правила, которые из этого следуют:

1. **Сессию не передают между задачами.** Открыл — закрыл в той же функции (или явно передал
   вниз по стеку вызова, как `db` в `maybe_announce_meta_gather_completed`).
2. **После `await` не полагаться на ORM-объект, прочитанный до него.** Отправка в Telegram и
   рисование картинок — это секунды. Значения, нужные после отправки, вынимать в примитивы
   заранее, а флаги-однократности ставить атомарным `UPDATE ... WHERE ... IS NULL` (см.
   `_reserve_announce` в `bot/scheduler.py`), а не мутацией объекта.
