...
Шлёт **один** анонс владельцу в личку (`settings.OWNER_CHAT_ID`): «🎉 Сбор метагейма завершён»,
счётчики участников и список игроков **без поражений (X-0)** с колодами (`get_undefeated_players`).
Идемпотентность — флаг `Tournament.completed_announced_at`, ставится **только после успешной
отправки** (сбой не «съедает» анонс — повторится при следующем импорте). После анонса турнир
**автоматически закрывается** (`close_tournament`, REGISTRATION → CLOSED; best-effort — сбой
закрытия анонс не роняет). Нет `OWNER_CHAT_ID` — пропускается.
