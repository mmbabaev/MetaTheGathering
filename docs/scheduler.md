...
`maybe_announce_meta_gather_completed()`. Турнир считается завершённым, когда у всех
не-бай матчей появился счёт (`AetherhubImportService.is_tournament_complete` — счёт
публикуется только после конца турнира, значит и финальные стендинги получены).
Шлёт **один** анонс владельцу в личку (`settings.OWNER_CHAT_ID`, НЕ в чат турнира —
пока так и в debug, и в prod; позже можно переключить на чат клуба): «🎉 Сбор
метагейма завершён», счётчики участников и список игроков **без поражений (X-0)** с их
колодами (`get_undefeated_players`). Идемпотентность — флаг
`Tournament.completed_announced_at`, который ставится **только после успешной отправки**
(сбой отправки не «съедает» анонс — повторится при следующем импорте). Турнир НЕ
закрывается автоматически. Best-effort: сбой не роняет джобу; нет `OWNER_CHAT_ID` —
пропускается.

К анонсу прикладываются **картинки** (все — `send_photo`, чтобы в чате было превью):
**график «Метагейм-срез»** (см. [`meta_chart.md`](meta_chart.md)) и **итоговые стендинги**
(см. [`standings.md`](standings.md), длинный список — несколько страниц).

Текст едет подписью первой картинки; если он длиннее лимита подписи в 1024 UTF-16-единицы
(много игроков без поражений) — уходит отдельным сообщением, а картинки следом.

**Картинки не могут сломать анонс.** Каждый рендер обёрнут в try/except (упал — просто нет
этой картинки). Текст доставляется гарантированно: либо подписью первой картинки (её отправку
НЕ глушим — падение пробрасывается, флаг не встаёт, анонс повторится), либо отдельным
сообщением, после которого картинки шлются best-effort.
