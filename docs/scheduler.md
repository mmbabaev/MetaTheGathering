...
К анонсу прикладывается **график «Метагейм-срез»** (см. [`meta_chart.md`](meta_chart.md)):
уходит документом (не фото — Telegram пережимает фото и легенда плывёт), текст идёт
подписью. Если текст длиннее лимита подписи в 1024 символа (много игроков без поражений) —
текст уходит отдельным сообщением, а график следом. **Картинка не может сломать анонс:**
рендер обёрнут в try/except, и при любой ошибке анонс уходит текстом, как раньше.

Данные для графика читаются в основном потоке, а само рисование (~180 мс CPU) уносится в
`asyncio.to_thread` — иначе оно заморозило бы event loop всему боту. Сессию SQLAlchemy
в поток передавать нельзя, поэтому `MetaChartService.prepare()` (БД) и `render_sectors()`
(чистый CPU) разделены.
