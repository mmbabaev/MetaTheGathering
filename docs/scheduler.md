...
- **Что делает:** снимает `decks_hidden` у незакрытых турниров, созданных сегодня (во время регистрации колоды скрыты, чтобы их не копировали). Затем постит **один** анонс в чат турнира: «👁 Колоды раскрыты», счётчики участников и короткую мету (топ колод из `StatsService.get_tournament_meta`). Анонс best-effort: сбой отправки не роняет джобу; нет `chat_id` — пропускается.
