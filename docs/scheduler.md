...
`AetherhubTimedImportJob`, `AetherhubFinalReimportJob`) через
...
Ночной `AetherhubFinalReimportJob` (06:00) — гарантированный бэкап: там >4 ч всегда, так что
если вечерние импорты почему-то не сработали, анонс уйдёт утром.
