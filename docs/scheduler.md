...
уходит картинкой (`send_photo`, чтобы в чате было превью), текст идёт подписью. Если текст
длиннее лимита подписи в 1024 символа (много игроков без поражений) — текст уходит отдельным
сообщением, а график следом. **Картинка не может сломать анонс:** рендер обёрнут в try/except,
и при любой ошибке анонс уходит текстом, как раньше.
