...
- **Что делает:** закрывает предыдущий активный турнир клуба → создаёт новый в статусе `REGISTRATION` → если задан `settings.ANNOUNCE_CHAT_ID`, шлёт в этот чат анонс «Турнир создан. Регистрация открыта».
...
- **Что делает:** снимает `decks_hidden` у незакрытых турниров, созданных сегодня (во время регистрации колоды скрыты, чтобы их не копировали). Затем шлёт **один** анонс в личку админу (`settings.ANNOUNCE_CHAT_ID`, тот же канал, что и анонс создания турнира — НЕ в чат турнира): «👁 Колоды раскрыты», счётчики участников и короткую мету (топ колод из `StatsService.get_tournament_meta`). Анонс best-effort: сбой отправки не роняет джобу; нет `ANNOUNCE_CHAT_ID` — пропускается.
