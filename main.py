    ...
    if isinstance(update, TGUpdate) and update.effective_message:
        try:
            await update.effective_message.reply_text(
                "⚠️ Внутренняя ошибка. Попробуйте ещё раз."
            )
        except Exception:
            pass
    ...
    try:
        ...
        for chat_id in settings.chat_ids:
            try:
                ...
                logger.warning(
                    f"[DEBUG] Could not create tournament for chat {chat_id}: {e}"
                )
    finally:
        db.close()
...
def main() -> None:
    ...
    app.add_handler(
        CommandHandler("tournaments", player.cmd_tournaments, filters=private)
    )
    app.add_handler(
        CommandHandler("settings", settings_handler.cmd_settings, filters=private)
    )
    ...
    app.add_handler(
        CommandHandler("add_players", admin.cmd_add_players, filters=private)
    )
    app.add_handler(
        CommandHandler(
            "tournament_status", admin.cmd_tournament_status, filters=private
        )
    )
    app.add_handler(CommandHandler("archive", admin.cmd_archive, filters=private))
    app.add_handler(
        CommandHandler(
            "create_tournament", admin.cmd_create_tournament, filters=private
        )
    )
    app.add_handler(
        CommandHandler(
            "delete_tournament", admin.cmd_delete_tournament, filters=private
        )
    )
    ...
    app.add_handler(
        CallbackQueryHandler(
            player.callback_tournament_select, pattern=f"^{CB_TOURNAMENT}:"
        )
    )
    app.add_handler(
    ...
    app.add_handler(
        CallbackQueryHandler(
            player.callback_archetype_more, pattern=f"^{CB_ARCHETYPE_MORE}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            player.callback_custom_archetype, pattern=f"^{CB_CUSTOM_ARCHETYPE}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            player.callback_tournament_status, pattern=f"^{CB_TSTATUS}:"
        )
    )
    app.add_handler(
    ...
    app.add_handler(
        CallbackQueryHandler(
            player.callback_leave_confirm, pattern=f"^{CB_LEAVE_CONFIRM}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            player.callback_leave_cancel, pattern=f"^{CB_LEAVE_CANCEL}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            settings_handler.callback_settings_name, pattern=f"^{CB_SETTINGS_NAME}$"
        )
    )
    app.add_handler(
    ...
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_admin_pick_arch, pattern=f"^{CB_ADMIN_PICK_ARCH}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_admin_set_arch, pattern=f"^{CB_ADMIN_SET_ARCH}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_admin_arch_more, pattern=f"^{CB_ADMIN_ARCH_MORE}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_admin_custom_arch, pattern=f"^{CB_ADMIN_CUSTOM_ARCH}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_export_excel, pattern=f"^{CB_EXPORT_EXCEL}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_delete_tournament_prompt, pattern=f"^{CB_DELETE_TOURNAMENT}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_delete_tournament_confirm,
            pattern=f"^{CB_DELETE_TOURNAMENT_CONFIRM}:",
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_delete_tournament_cancel,
            pattern=f"^{CB_DELETE_TOURNAMENT_CANCEL}:",
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_admin_show_filled, pattern=f"^{CB_ADMIN_SHOW_FILLED}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_reveal_decks, pattern=f"^{CB_REVEAL_DECKS}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            poll_handler.callback_poll_menu, pattern=f"^{CB_POLL_MENU}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            poll_handler.callback_link_poll_prompt, pattern=f"^{CB_LINK_POLL_BY_URL}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            poll_handler.callback_create_poll, pattern=f"^{CB_CREATE_POLL}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            poll_handler.callback_notify_no_deck, pattern=f"^{CB_NOTIFY_NO_DECK}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            poll_handler.callback_notify_confirm, pattern=f"^{CB_NOTIFY_CONFIRM}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            poll_handler.callback_notify_cancel, pattern=f"^{CB_NOTIFY_CANCEL}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            aetherhub_handler.callback_aetherhub_import_prompt,
            pattern=f"^{CB_AETHERHUB_IMPORT}:",
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            aetherhub_handler.callback_aetherhub_confirm,
            pattern=f"^{CB_AETHERHUB_CONFIRM}:",
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            aetherhub_handler.callback_aetherhub_cancel,
            pattern=f"^{CB_AETHERHUB_CANCEL}:",
        )
    )
    app.add_handler(
    ...
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_close_tournament, pattern=f"^{CB_CLOSE_TOURNAMENT}:"
        )
    )
    app.add_handler(
        CallbackQueryHandler(
            admin.callback_admin_opponents, pattern=f"^{CB_ADMIN_OPPONENTS}:"
        )
    )
    ...
    app.add_handler(
        MessageHandler(
            filters.TEXT & ~filters.COMMAND & private, player.message_text_input
        )
    )
