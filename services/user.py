...
class UserService:
    ...
    def is_tournament_organizer(self, tg_id: int) -> bool:
        user = self.get_by_tg_id(tg_id)
        return user is not None and bool(user.is_tournament_organizer)

    def can_create_tournament(self, tg_id: int) -> bool:
        return self.is_admin(tg_id) or self.is_tournament_organizer(tg_id)

    def can_manage_tournament(self, tg_id: int, tournament: models.Tournament) -> bool:
        """Admins manage all events; draft organizers manage drafts they created."""
        return self.is_admin(tg_id) or (
            tournament.is_draft and tournament.created_by_tg_id == tg_id and self.is_tournament_organizer(tg_id)
        )
