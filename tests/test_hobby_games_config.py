...
def test_hobby_games_identity():
    identity = next(row for row in club_identities() if row.name == "Hobby Games - Калининград")
...
def test_hobby_games_defaults_follow_pair_of_dice_registration_flow():
    schedule = next(row for row in default_schedules() if row.club_name == "Hobby Games - Калининград")
...
def test_hobby_games_default_club_uses_local_timezone():
    club = next(row for row in default_clubs() if row.name == "Hobby Games - Калининград")
