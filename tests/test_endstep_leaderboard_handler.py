...
def test_pages_show_ru_and_site_positions(monkeypatch):
    ...
    assert "<pre>RU Ник" in first.text
    assert " 1 Player01" in first.text
    assert "    3 1700  70  10-4-1" in first.text
    assert "10 Player10" in first.text
    assert "11 Player11" not in first.text
    assert "11 Player11" in second.text
    assert "  —* 1690  70  10-4-1" in second.text
    assert "provisional" in second.text
    assert "R — рейтинг, RD — отклонение" in second.text
...
def test_html_table_escapes_endstep_usernames(monkeypatch):
    monkeypatch.setattr("bot.handlers.endstep_leaderboard.settings.OWNER_CHAT_ID", OWNER_ID)
    ...
    assert "same&amp;name" in result.text
