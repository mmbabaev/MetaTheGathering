...
- Отдельный beta-режим внутреннего Swiss-движка для онлайн-турниров: администратор
  ...
  pair-up/pair-down. Рекомендуемое число Swiss-раундов зависит от числа игроков
  (`max(3, ⌈log₂ N⌉)`, от 3 до 20, до первого раунда ручная настройка).
...
- Large онлайн-турниры во внутреннем Swiss: опциональный single-elimination плей-офф
  топ-8/топ-16 (`⚙️ Настройки Swiss` до старта). Брекеты фиксированы, меньший seed
  ходит первым; плей-офф добавляет раунды по `⌈log₂ size⌉`, последний — финал и матч
  за 3-е место. Итоговые места: 1–2 из финала, 3–4 из матча за 3-е место, выбывшие в
  одном раунде получают целые места по сиду, остальные — по Swiss-порядку после ката.
  ...
  влияют на Swiss-стендинги — PR
  [#332](https://github.com/mmbabaev/MetaTheGathering/pull/332).
