...
- Доработка `/schedule` из [#125](https://github.com/mmbabaev/MetaTheGathering/issues/125):
  день создания турнира можно выбрать от дня события до шести дней заранее; карточка и
  общий список явно показывают фактический день создания, включая питерское «накануне».
