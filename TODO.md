...
- Первый read-only этап Moscow Pauper Ranked из
  ...
  Для будущей публичной таблицы активацией считаются только источники `self_bot` и `cellar`;
  самостоятельная бронь Cellar равна самостоятельной регистрации, `admin` и `aetherhub_import`
  права на публичное отображение не дают.
