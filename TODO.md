...
- Owner-only напоминания о незакрытом турнире через 3 и 7 дней содержат прямую ссылку AetherHub,
  если она привязана к турниру; состав получателей и расписание рассылки не меняются — PR
  [#266](https://github.com/mmbabaev/MetaTheGathering/pull/266).

- Колоды из ячейки из
  [#260](https://github.com/mmbabaev/MetaTheGathering/issues/260): основной Telegram-flow
  команды `/cellar` показывает четыре ближайших турнира «Единорога» по понедельникам и четвергам,
  позволяет выбрать физическую копию, открыть её деклист, забронировать или отменить бронь.
  Каталог показывает по 10 колод, закрепляя сверху активную
  бронь и до трёх последних уникальных выборов; web-каталог сохранён как альтернативный вход.
  Архетип автоматически записывается в турнир. Мгновенные DM о бронях/отменах получает только
  владелец; за час до турнира Иван, Сергей и владелец получают одну сводку с именами,
  Telegram-никами и физическими колодами. Ивану, Сергею и владельцу тот же список доступен в
  `/cellar`; весь flow закрыт выключенным по умолчанию `cellarDecks`. Source of truth —
  переданная публичная Google-таблица: названия, физические копии с номером строки, ссылки, дата
  актуальности и пометки доступности синхронизируются автоматически по воскресеньям в 23:00 МСК;
  ни Telegram-, ни web-каталог не делают сетевой запрос при открытии. PR
  [#261](https://github.com/mmbabaev/MetaTheGathering/pull/261) создан напрямую от `main`.
