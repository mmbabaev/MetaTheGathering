...
- Персональное меню Telegram повторно синхронизируется при обычном `/start`, поэтому новая
  роль организатора турниров получает `/create_tournament`, даже если первичная регистрация
  команд при деплое не удалась или бот ещё не знал личный чат пользователя. Ошибки стартовой
  синхронизации ролей больше не скрываются молча — PR
  [#327](https://github.com/mmbabaev/MetaTheGathering/pull/327).
