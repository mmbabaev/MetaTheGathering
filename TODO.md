...
- Внутренний Swiss для `Endstep-ru` показывает заявленные колоды обоих игроков после
  отправки результата. В публичных парингах у каждого игрового стола есть своя кнопка
  копирования названия `MTG Pauper Endstep <игрок 1> - <игрок 2>`, а в личном экране
  автоматически выбирается текущая пара игрока. BYE и остальные клубы не меняются — PR
