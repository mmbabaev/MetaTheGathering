...
- Исправление [#318](https://github.com/mmbabaev/MetaTheGathering/issues/318) проходит review:
  live-страница AetherHub больше не может выдать номера столов за имена из-за позиционного
  разбора временной таблицы; standings и колонка `Name` определяются по заголовкам, а импорт
  дополнительно отвергает значения без букв как личности игроков.
...
- Для онлайн-турниров собираются результаты текущего раунда: игрок вводит свои победы и победы
  ...
  В debug-боте доступны 7 тестовых игроков и Swiss-подобные раунды; игроки и администраторы
  могут дропаться во время Swiss, администратор может закрыть Swiss из меню, а кнопка под
  публичной карточкой раунда ведёт игрока к текущему турниру или сразу к вводу результата.
...
- Telegram- и Endstep-ники поддерживаются для сопоставления импортов и публикации парингов, но
  ...
  ФИО, текущие счёты и статусы матчей. Опубликованное сообщение автоматически редактируется после
  отправки результата на подтверждение, подтверждения, отклонения и админского исправления — PR
  [#281](https://github.com/mmbabaev/MetaTheGathering/pull/281),
  [#287](https://github.com/mmbabaev/MetaTheGathering/pull/287),
  [#288](https://github.com/mmbabaev/MetaTheGathering/pull/288).
- Профиль пользователя хранит необязательный город. В `/settings` доступны быстрые
  варианты «Москва» и «Санкт-Петербург», а также ручной ввод любого города — PR
  [#296](https://github.com/mmbabaev/MetaTheGathering/pull/296).
...
- Swiss-стендинги сохраняют очки и тай-брейки уже сыгранного матча, если соперник
  позже дропнулся; сам дропнутый игрок по-прежнему не участвует в следующих парингах — PR
  [#297](https://github.com/mmbabaev/MetaTheGathering/pull/297).
...
- Первый этап Endstep Pauper leaderboard из
  ...
  в штатном режиме `Continue as Guest`, находит текущий Pauper queue и ищет несколько
...
- Метаписцы получили в карточке активного турнира действия добавления
  ...
  [#320](https://github.com/mmbabaev/MetaTheGathering/pull/320).
- `Endstep draft`: отдельный тип онлайн-турнира и отдельная роль организатора. Драфт
  создаётся сразу на внутреннем Swiss в заданном для окружения чате, принимает игроков
  без архетипа и не рассылает напоминания о необязательных деклистах. До первого раунда
  организатор отдельной кнопкой фиксирует случайную рассадку; после неё регистрация закрыта,
  но деклисты ещё редактируются. Первый из трёх раундов запускается второй кнопкой и сводит
  противоположные половины рассадки (`1–5`, `2–6`, `3–7`, `4–8` для восьми игроков) — PR
  [#325](https://github.com/mmbabaev/MetaTheGathering/pull/325).

Игрокам ачивки автоматически не рассылаются: текущий режим — owner-only shadow.

### В review, но ещё не в `main`

- Регулярные онлайн-турниры `Endstep-ru` публично переименованы в «Концеход»: заголовок
  и анонс открытия регистрации нумеруются сквозным счётчиком прошлых турниров клуба
  (`⏭️🦶 Концеход Pauper #N`, дата ушла из названия — она остаётся в тексте анонса).
  Паринги, напоминание перед стартом и анонс завершения показывают сохранённый
  заголовок. Клуб в БД, кнопка копирования стола `MTG Pauper Endstep - table`,
  лидерборды и `Endstep draft` не меняются. Активный турнир в проде миграцией
  переименован в «Концеход Pauper #3». Анонс записи умеет уходить фото с иконкой
  (`KONETSKHOD_ICON_PATH`, файл `assets/tournament-icons/konetskhod.png`; без файла —
  текст как раньше; счётчик записавшихся обновляется через `edit_message_caption`).

- Концеход (регулярный онлайн-турнир `Endstep-ru`) рассылает уведомления о столе
  и оппоненте в личку всем самозарегистрированным игрокам раунда — настройка
  «Уведомления об оппоненте» игнорируется, gate `notify_allowed_ids` остаётся.
  В клубных парингах Концехода вместо Telegram-ников показываются Endstep-ники,
  при их отсутствии — прочерк. Остальные клубы не меняются.

- В мастере создания Концехода добавлен необязательный шаг «Название турнира»:
  пустой ввод оставляет автогенерируемый заголовок `⏭️🦶 Концеход Pauper #N`,
  свой текст (до 64 символов) используется вместо него в анонсе, парингах и
  напоминаниях. Для остальных клубов шаг не показывается.

- В online-турнирах роль метаписца больше не открывает организаторский список игроков,
  заявленные колоды и административные действия. Метаписец получает ту же карточку и права,
  что обычный игрок; его прежние возможности сохраняются только для offline-турниров — PR
  [#326](https://github.com/mmbabaev/MetaTheGathering/pull/326).

- Внутренний Swiss получает plain-text деклисты: самостоятельная запись требует архетип,
  игрок загружает и меняет список до старта, создатель турнира и владелец видят все списки
  до закрытия, а после финальных стендингов список игроков и их деклисты доступны всем через
  существующую кнопку «Статус». За 15 минут до заданного времени начала новые турниры
  адресно напоминают только игрокам без архетипа или деклиста; существующие турниры миграцией
  исключены из новой рассылки — PR
  [#324](https://github.com/mmbabaev/MetaTheGathering/pull/324).

- Внутренний Swiss для `Endstep-ru` показывает заявленные колоды обоих игроков после
  отправки результата. В публичных парингах и личном экране доступна одна кнопка,
  копирующая статическое название стола `MTG Pauper Endstep - table`. Остальные клубы
  не меняются — PR
  [#322](https://github.com/mmbabaev/MetaTheGathering/pull/322).

- Метаписец может из статуса турнира открыть меню конкретного игрока и удалить
...
- Moscow Pauper Ranked из [#306](https://github.com/mmbabaev/MetaTheGathering/issues/306):
  ...
  20 июня — 19 сентября, а публичный `/leaderboard` продолжает текущее состояние с 20 июня
  без сброса 20 сентября и показывает активных игроков страницами по десять
  из закрытых полных турниров бота в Goldfish и Edinorog. Зеркальные паринги и повторный
  AetherHub source дедуплицируются, bye/неполные матчи исключаются, identity поддерживает
  обратный порядок ФИО, инициалы и одну однозначную опечатку внутри состава. Активацией
  публичной таблицы считаются отдельные новые события `self_bot` и `cellar`:
  самостоятельная регистрация, самостоятельное указание своей колоды после импорта или бронь
  Cellar. Создание участия через `admin` и `aetherhub_import` само по себе права на публичное
  отображение не даёт. Публичность включается только после закрытия первого зачётного турнира
  с самостоятельным действием; выход до закрытия не открывает рейтинг. Каждый последующий
  зачётный турнир без самостоятельного действия оставляет игрока публичным и добавляет
  настраиваемые −10 к публичному Score. Включённый по умолчанию `rankedPublic` добавляет
  рейтинговый абзац в существующие адресные уведомления: скрытый оппонент показывается без
  Score и прогноза. Каждый реальный матч даёт обоим игрокам `+1` только к публичному Score;
  прогноз win/draw/lose показывает уже итоговое публичное изменение с этим бонусом. Bye и
...
- Защита инцидента [#304](https://github.com/mmbabaev/MetaTheGathering/issues/304): один числовой
  ...
  автоматический, timed, ручной и CLI-пути останавливаются до изменения состава и парингов — PR
  [#305](https://github.com/mmbabaev/MetaTheGathering/pull/305).

- В персональном режиме админ-статуса «по парингам» номер стола добавляется к
  кнопке первого игрока, а полностью заполненные столы по умолчанию скрываются и
  раскрываются одной кнопкой — PR
  [#150](https://github.com/mmbabaev/MetaTheGathering/pull/150).

- Калининградский клуб во всех интерфейсах и новых турнирах называется
  `Калининград`; существующие турниры, расписания и клубные настройки
  переименовываются миграцией без потери данных — PR
  [#301](https://github.com/mmbabaev/MetaTheGathering/pull/301).

- При завершении внутреннего Swiss финальные стендинги публикуются в чат турнира
  текстом вместе с графиком меты и картинками стендингов. Для Endstep-ru город
  Magic Oculus настроен как `Endstep`; до первой загрузки соответствующие город и
  клуб должны появиться во внешнем справочнике Oculus — PR
  [#299](https://github.com/mmbabaev/MetaTheGathering/pull/299).

- В архивной карточке завершённого внутреннего Swiss кнопка «Статус» показывает
  копируемые текстовые стендинги вместо управления результатами; действия нового
  раунда, редактирования результатов и повторного завершения скрыты. В меню
  «Выгрузка» добавлен текстовый список «игрок — Telegram-ник — очки — город» — PR
  [#300](https://github.com/mmbabaev/MetaTheGathering/pull/300).

- Отдельный beta-режим внутреннего Swiss-движка для онлайн-турниров: администратор
  может включить его в debug- и production-боте; кнопка доступна только администраторам.
  Администратор создаёт первый и следующие раунды после сбора результатов, а бот считает официальные
  match points и OMW/GW/OGW, выдаёт bye снизу без повтора, избегает rematch и балансирует
  pair-up/pair-down. На этапе beta независимо от числа игроков фиксируется 4 раунда.
  ...
  [#291](https://github.com/mmbabaev/MetaTheGathering/pull/291).
