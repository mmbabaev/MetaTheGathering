...
- Первый read-only этап Moscow Pauper Ranked из
  ...
  отображение не даёт. В уведомлении о раунде Score оппонента и прогноз `win/draw/lose`
  показываются только для публично активированного оппонента; для скрытого выводится только
  собственный Score и текст «Рейтинг соперника скрыт».
