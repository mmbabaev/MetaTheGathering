...
- Лимит двух активных турниров считается по клубу, а не по техническому адресу объявления:
  `chat_id=0` («не отправлять») и общий debug-чат больше не связывают разные клубы в один лимит.
...
- Для онлайн-турниров собираются результаты текущего раунда: игрок вводит свои победы и победы
  ...
  Неподтверждённый счёт публично виден с `⏳`; администратор может задать или исправить результат
  без peer-confirmation и получить текстовую сводку для ручного ввода в AetherHub. Вид публичного
  статуса (`список игроков` / `паринги текущего раунда`) выбирается для каждого турнира отдельно.
  В debug-боте турнир можно безопасно заполнить до 15 тестовых игроков и последовательно создавать
  Swiss-подобные раунды с рандомным завершением предыдущего раунда; тестовым игрокам сообщения не
  отправляются — PR [#284](https://github.com/mmbabaev/MetaTheGathering/pull/284).
- Онлайн-регистрация требует уникальный Endstep-ник, который игрок может изменить в `/settings`.
  Для каждого клуба добавлен выключенный по умолчанию флаг публикации новых парингов в чат;
  ручной и оба автоматических импорта AetherHub используют один формат с Telegram- и Endstep-никами — PR
