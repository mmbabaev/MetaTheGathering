...
- Финальная очистка состава после импорта AetherHub удаляет зарегистрированных в боте no-show
  только когда опубликованы standings, завершены все матчи и у каждого игрока из AetherHub уже
  заполнена колода. Та же очистка по сохранённым standings и парингам выполняется перед публикацией,
  поэтому позднее заполнение последней колоды не включает bot-only игрока в результаты. Пока хотя
  бы одна колода AetherHub-участника не указана, состав не сокращается и турнир не публикуется — PR
  [#263](https://github.com/mmbabaev/MetaTheGathering/pull/263).
