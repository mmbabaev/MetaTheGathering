...
- Для онлайн-турниров собираются результаты текущего раунда: игрок вводит свои победы и победы
  ...
  публичной карточкой раунда ведёт игрока к текущему турниру или сразу к вводу результата — PR
...
- Moscow Pauper Ranked из [#306](https://github.com/mmbabaev/MetaTheGathering/issues/306):
  ...
  отображение не даёт. Два рейтинговых турнира подряд без самостоятельного действия скрывают
  игрока и добавляют −50 только к публичному Score; штраф не повторяется до новой активации,
  после которой начинается новый цикл. Включённый по умолчанию `rankedPublic` добавляет
  рейтинговый абзац в существующие адресные уведомления: скрытый оппонент показывается без
  Score и прогноза. Каждый реальный матч даёт обоим игрокам `+0.5` только к публичному Score;
