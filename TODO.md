...
- Отдельный beta-режим внутреннего Swiss-движка для онлайн-турниров: администратор
  создаёт четыре раунда после сбора результатов, а бот считает match points и
  OMW/GW/OGW, выдаёт BYE без повтора, избегает rematch и балансирует pair-up/pair-down.
  AetherHub остаётся дефолтным изолированным режимом — PR
...
- При завершении внутреннего Swiss финальные стендинги публикуются в чат турнира
  текстом вместе с графиком меты и картинками стендингов. Архивная карточка показывает
  копируемые стендинги, а выгрузка — список «игрок — Telegram-ник — очки — город».
  Клуб Endstep-ru сопоставлен городу Magic Oculus `Endstep` — PR
  [#299](https://github.com/mmbabaev/MetaTheGathering/pull/299),
  [#300](https://github.com/mmbabaev/MetaTheGathering/pull/300).
- Калининградский клуб во всех интерфейсах называется `Калининград`; прежние данные
  переименованы миграцией без потери связей — PR
...
- Закрытый внутренний Swiss собирается из собственных стендингов и матчей в два
  ...
  Автоматическая отправка защищена отдельным default-off feature flag. До merge
  нужен подтверждённый контракт колонок CSV со стороны Oculus — draft PR
