...
- Внутренний Swiss получает plain-text деклисты: самостоятельная запись без колоды возможна
  только через «Укажу позже» в прежнее окно, игрок загружает и меняет список до старта, создатель
  турнира и владелец видят все списки
...
- Отдельный beta-режим внутреннего Swiss-движка для онлайн-турниров: администратор
  ...
  pair-up/pair-down. Формат — per-турнирная настройка до первого раунда: классический
  по умолчанию (4 раунда, без плей-оффа) или большой (раунды по числу игроков).
...
- Формат большого онлайн-турнира во внутреннем Swiss (per-турнирный opt-in через
  `⚙️ Настройки Swiss`, до первого раунда): рекомендуемое число раундов по полю
  (`max(3, ⌈log₂ N⌉)`, 3–20, ручная настройка) и опциональный single-elimination
  плей-офф топ-8/топ-16. Брекеты фиксированы, меньший seed ходит первым;
  плей-офф добавляет раунды по `⌈log₂ size⌉`, последний — финал и матч за 3-е место.
  Итоговые места: 1–2 из финала, 3–4 из матча за 3-е место, выбывшие в одном раунде
  получают целые места по сиду, остальные — по Swiss-порядку после ката.
  Ничьи в плей-офф-матчах отклоняются, после старта плей-оффа дроп запрещён, при
  малом поле плей-офф автоматически не играется (финиш по Swiss). Плей-офф-матчи не
  влияют на Swiss-стендинги. Классический формат не меняется: 4 раунда без топа —
  PR [#332](https://github.com/mmbabaev/MetaTheGathering/pull/332).
