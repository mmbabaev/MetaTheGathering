...
- Ручная кнопка AetherHub использует ту же проверку даты игрового дня, что и плановый импорт.
  Если точного совпадения нет, старое событие не предлагается, а админ получает ссылку на Content Feed клуба.
  После второй плановой попытки без турнира владелец бота один раз получает такое же сообщение в личку — PR
