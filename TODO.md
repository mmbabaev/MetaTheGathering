...
- Внутренний Swiss для `Endstep-ru` добавляет заявленные колоды обоих игроков в
  сообщения результата. В публичных парингах и личном экране доступна одна кнопка,
  копирующая статическое название стола `MTG Pauper Endstep - table` — PR
...
- Уточнение показа колод в результате Endstep Swiss: предварительный экран «Вы указали»
  содержит только игроков и счёт; обе заявленные колоды раскрываются обеим сторонам
  только после нажатия «Передать» — PR
  [#323](https://github.com/mmbabaev/MetaTheGathering/pull/323).
