...
- Колоды из ячейки из
  ...
  Архетип автоматически записывается в турнир, о бронях/отменах уходят адресные DM. В
  production уведомления и предтурнирную сводку получают Иван, Сергей и владелец, в debug —
  только владелец; весь flow закрыт выключенным по умолчанию `cellarDecks`. Source of truth —
