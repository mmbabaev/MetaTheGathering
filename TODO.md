...
- Первый read-only этап Moscow Pauper Ranked из
  [#306](https://github.com/mmbabaev/MetaTheGathering/issues/306): admin-only
  `/ranked_preseason` пересчитывает Glicko-2 top-10 за 20 июня — 19 сентября только
  ...
  Для будущей публичной таблицы активацией считаются отдельные события `self_bot` и `cellar`:
  ...
  после которой начинается новый цикл. Рейтинговый абзац в адресных уведомлениях закрыт
  выключенным по умолчанию `rankedPublic`: скрытый оппонент показывается без Score и прогноза.
