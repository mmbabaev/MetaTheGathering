...
Публичный список изначально закрыт для игрока. Он появляется в нём после самостоятельной записи на рейтинговый турнир через бота, самостоятельного указания своей колоды после импорта или брони колоды Cellar. Добавление администратором и импорт AetherHub сами по себе не открывают рейтинг.

Если игрок участвует в двух рейтинговых турнирах подряд без самостоятельного действия, он скрывается из таблицы и получает −50 к публичному Score. Пока рейтинг скрыт, штраф не повторяется. Новая самостоятельная запись возвращает игрока; накопленный штраф остаётся, а следующие два пропуска образуют новый цикл.
