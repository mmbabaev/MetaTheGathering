...
CITY_INVALID = "Город должен быть одной непустой строкой длиной до 255 символов."
...
SETTINGS_CITY_PROMPT = "🏙 Выберите город:"
SETTINGS_CITY_CUSTOM_PROMPT = "Введите свой город:"
...
/leaderboard — публичный Moscow Pauper Ranked

/settings — сохранить имя и фамилию для автоматической записи\
"""

RANKED_RULES_TEXT = """\
📖 Правила Moscow Pauper Ranked

Что попадает в рейтинг
Учитываются завершённые Pauper-турниры Goldfish и Edinorog начиная с 20 июня 2026 года. Источник — паринги и результаты, сохранённые в боте. Bye не считается матчем. Незавершённые турниры, неполные результаты и повторный импорт одного AetherHub-турнира исключаются.

Как открыть свой рейтинг
Публичный список изначально закрыт для игрока. Самостоятельная запись на рейтинговый турнир через бота, самостоятельное указание своей колоды после импорта или бронь колоды Cellar сохраняют право на открытие рейтинга. Игрок появляется в таблице только после закрытия этого турнира и обработки полных результатов. Если он вышел до турнира, запись не учитывается. Добавление администратором и импорт AetherHub сами по себе рейтинг не открывают.

После открытия рейтинга каждый следующий сыгранный рейтинговый турнир без самостоятельного действия игрока добавляет −10 к публичному Score. Игрок остаётся в таблице, а накопленный штраф сохраняется. Самостоятельное действие в следующем турнире освобождает от нового штрафа, но не отменяет предыдущие.

Как считается рейтинг
Используется Glicko-2 с недельными периодами. У каждого игрока есть рейтинг R, неопределённость RD и волатильность σ. Победа даёт результат 1, ничья 0,5, поражение 0. Изменение зависит от рейтинга и RD обоих соперников: победа над сильным или над игроком с надёжным рейтингом обычно влияет сильнее.

Переход к шкале Glicko-2:
μ = (R − 1500) / 173,7178
φ = RD / 173,7178

Для каждого соперника j:
g(φⱼ) = 1 / √(1 + 3φⱼ² / π²)
Eⱼ = 1 / (1 + exp(−g(φⱼ)(μ − μⱼ)))
v = 1 / Σ[g(φⱼ)² Eⱼ(1 − Eⱼ)]
Δ = v Σ[g(φⱼ)(sⱼ − Eⱼ)]

Затем по стандартному итерационному шагу Glicko-2 обновляется σ с параметром τ = 0,5, после чего:
φ* = √(φ² + σ'²)
φ' = 1 / √(1/φ*² + 1/v)
μ' = μ + φ'² Σ[g(φⱼ)(sⱼ − Eⱼ)]
R' = 173,7178μ' + 1500
...
За каждый реально сыгранный матч оба игрока получают небольшой бонус участия +1. Bye и несыгранные после drop раунды бонуса не дают. Бонус влияет только на публичный Score и не меняет скрытые R, RD и σ.

Публичное число в таблице:
Score = round(R − 2 × RD + матчи) − публичные штрафы.
...
def format_opponent_notification(
    ...
    *,
    ...
    if ranked_own_score is not None:
        ...
        lines.append("Бонус за сыгранный матч: +1 к публичному Score.")
